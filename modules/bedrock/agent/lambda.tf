...
resource "aws_lambda_function" "bi_chatbot_container_lambda" {
  ...
  memory_size  = 1024
  layers = [
    "arn:aws:lambda:${local.region}:${local.account_id}:layer:AWSLambdaPowertoolsPythonV3:1"
  ]
  tracing_config {
    mode = "Active"
  }
...
resource "null_resource" "lambda_code_update" {
  ...
  provisioner "local-exec" {
    command = "aws lambda update-function-code --function-name ${aws_lambda_function.bi_chatbot_container_lambda.function_name} --s3-bucket ${var.code_base_bucket} --s3-key ${var.code_base_zip}  --region ${local.region} --output text"
  }
